...
import type { AdminSessionPayload, DemeterReportQueueSnapshot } from "@/lib/types"

export type DemeterReportQueueWebSocketMode = "polling" | "websocket_auth" | "websocket"

type DemeterReportQueueWebSocketMessage =
  | { type: "auth_ok" }
  | { type: "auth_error"; code?: string; message?: string }
  | { type: "snapshot"; snapshot?: DemeterReportQueueSnapshot }
  | { type: "command_ok"; commandId?: string; snapshot?: DemeterReportQueueSnapshot }
  | { type: "command_error"; commandId?: string; code?: string; message?: string }

type PendingCommand = {
  resolve: (snapshot: DemeterReportQueueSnapshot) => void
  reject: (error: Error) => void
}

type UseDemeterReportQueueWebSocketInput = {
  limit: number
  onSnapshot: (snapshot: DemeterReportQueueSnapshot) => void
  refreshSession: () => Promise<AdminSessionPayload | null>
}

function toDemeterReportQueueWebSocketUrl(limit: number) {
  const base = getRuntimeConfig().backendBaseUrl
  const suffix = `/admin/providers/demeter-sante/report-queue/ws?limit=${encodeURIComponent(String(limit))}`
...
export function useDemeterReportQueueWebSocket({
  limit,
  onSnapshot,
  refreshSession,
}: UseDemeterReportQueueWebSocketInput) {
  const [mode, setMode] = useState<DemeterReportQueueWebSocketMode>("polling")
  ...
  useEffect(() => {
    ...
    const handleMessage = (socket: WebSocket, raw: MessageEvent) => {
      let message: DemeterReportQueueWebSocketMessage
      try {
        message = JSON.parse(String(raw.data)) as DemeterReportQueueWebSocketMessage
      } catch {
        return
      }
    ...
    function openSocket() {
      ...
      const socket = new WebSocket(toDemeterReportQueueWebSocketUrl(limit))
  ...
  const updateSettings = useCallback((input: { parallelism: number }) => {
    ...
    return new Promise<DemeterReportQueueSnapshot>((resolve, reject) => {
